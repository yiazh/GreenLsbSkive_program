...
for t in range(total_cycle * 2, total_cycle * 3):
    A_eq_gls[t][
        t] = 24 / total_cycle * 25  # assuming the efficiency is constant (25kg/Mwh)so that we can use linear programming
    if t == 2 * total_cycle:
        A_eq_gls[t][t + 3 * total_cycle] = -1
        B_eq_gls[t] = 0
    else:
...
for t in range(0, total_cycle):
    A_ineq_gls[t][t + total_cycle] = 1 #battery
    A_ineq_gls[t][t + 2 * total_cycle] = 1  # electrolyser
    B_ineq_gls[t] = max(0,power_wind_t[t] + power_pv_t[t] - np.array(load_data_p_mw).sum())  # re power - load
